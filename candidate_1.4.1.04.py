...
init = True
model_name = "model_s1.4.1.05b"
# 0.0.4.01 - starting from scratch
# 0.0.4.02 - adding extra conv layers
# 0.0.4.03 - changed padding of pools to valid, added extra fc layer
# 0.4.1.04 - changed a bunch of stuff
...
with graph.as_default():
    ...
    with tf.name_scope('fc3') as scope:
        fc3 = tf.layers.dense(
            ...
            activation=None,  # None
            kernel_initializer=tf.variance_scaling_initializer(scale=2, seed=1191),
            bias_initializer=tf.zeros_initializer(),
            kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=lamF),
            name="fc3"
        )
        ...
        fc3 = tf.nn.relu(fc3, name='fc4_relu')

        # dropout
        fc3 = tf.layers.dropout(fc3, rate=fcdropout_rate, seed=120, training=training)
