...
# 1.0.4.01 - added a branch to layer 3
# 1.0.5.01 - took out branch 4
# 1.0.6.01 - added some skip connections
...
with graph.as_default():
    ...
    with tf.name_scope('conv2.4') as scope:
        conv24 = tf.layers.conv2d(
            pool1,  # Input data
            filters=48,  # 64 filters
            kernel_size=(1, 1),  # Kernel size: 3x3
            strides=(1, 1),  # Stride: 1
            padding='SAME',  # "same" padding
            activation=None,  # None
            kernel_initializer=tf.truncated_normal_initializer(stddev=5e-2, seed=104),
    ...
    with tf.name_scope('conv3.0') as scope:
        ...
        # apply relu
        conv3 = tf.nn.relu(conv3, name='relu3.0')
    ...
    with tf.name_scope('conv3.3') as scope:
        ...
        conv33 = tf.layers.batch_normalization(
            conv33,
            axis=-1,
            momentum=0.99,
            epsilon=epsilon,
        ...
        # apply relu
        conv33 = tf.nn.relu(conv33, name='relu3.3')
    ...
    # Convolutional layer 4.0
    with tf.name_scope('conv4.0') as scope:
        conv4 = tf.layers.conv2d(
            pool3,  # Input data
            filters=256,  # 256 filters
            kernel_size=(1, 1),  # Kernel size: 3x3
            strides=(1, 1),  # Stride: 1
            padding='SAME',  # "same" padding
            activation=None,  # None
        ...
        # apply relu
        conv4 = tf.nn.relu(conv4, name='relu4.0')

    # Convolutional layer 4.1
    with tf.name_scope('conv4.1') as scope:
        conv4 = tf.layers.conv2d(
            conv4,  # Input data
            filters=384,  # 256 filters
            kernel_size=(3, 3),  # Kernel size: 3x3
            strides=(1, 1),  # Stride: 1
            padding='SAME',  # "same" padding
            activation=None,  # None
    ...
    with tf.name_scope('conv5.1') as scope:
        ...
        # apply relu
        conv5_bn_relu = tf.nn.relu(conv5, name='relu5.1')
