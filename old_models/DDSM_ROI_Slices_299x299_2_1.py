...
init = True
model_name = "model_s0.0.2.01"
# 0.0.2.01 - trying an inception like model
...
with graph.as_default():
    ...
    with tf.name_scope('conv1') as scope:
        ...
        if dropout:
            conv1_bn_relu = tf.layers.dropout(conv1_bn_relu, rate=convdropout_rate, seed=200, training=training)
    ...
    with tf.name_scope('conv1.1') as scope:
        ...
        # optional dropout
        if dropout:
            conv11_bn_relu = tf.layers.dropout(conv11_bn_relu, rate=convdropout_rate, seed=400, training=training)
    ...
    with tf.name_scope('conv1.2') as scope:
        ...
        # apply relu
        conv12_bn_relu = tf.nn.relu(conv12, name='relu1.2')

        # optional dropout
        if dropout:
            conv12_bn_relu = tf.layers.dropout(conv12_bn_relu, rate=convdropout_rate, seed=600, training=training)

    # Max pooling layer 1
    with tf.name_scope('pool1') as scope:
        pool1 = tf.layers.max_pooling2d(
            conv12_bn_relu,  # Input
    ...
    with tf.name_scope('pool1.1') as scope:
        pool11 = tf.layers.conv2d(
            conv12_bn_relu,  # Input data
        ...
        pool11 = tf.nn.relu(pool11, name='relu_p1.1')

        # optional dropout
        if dropout:
            pool11 = tf.layers.dropout(pool11, rate=pooldropout_rate, seed=700, training=training)
    ...
    with tf.name_scope('branch2.2') as scope:
        ...
        with tf.name_scope('conv2.2.2') as scope:
            ...
            # apply relu
            conv222 = tf.nn.relu(conv222, name='relu2.2.2')
    ...
    # Branch 3
    with tf.name_scope('branch2.3') as scope:
        with tf.name_scope('conv2.3.1') as scope:
            conv231 = tf.layers.conv2d(
                concat1,  # Input data
                filters=64,  # 32 filters
                kernel_size=(1, 1),  # Kernel size: 9x9
                strides=(1, 1),  # Stride: 1
            ...
            conv231 = tf.layers.batch_normalization(
                conv231,
                axis=-1,
                momentum=0.99,
                epsilon=epsilon,
            ...
            conv231 = tf.nn.relu(conv231, name='relu2.3.1')

        with tf.name_scope('conv2.3.2') as scope:
            conv232 = tf.layers.conv2d(
                conv231,  # Input data
                filters=96,  # 32 filters
                kernel_size=(3, 3),  # Kernel size: 9x9
                strides=(1, 1),  # Stride: 1
            ...
            conv232 = tf.layers.batch_normalization(
                conv232,
                axis=-1,
                momentum=0.99,
                epsilon=epsilon,
            ...
            # apply relu
            conv232 = tf.nn.relu(conv232, name='relu2.3.2')

        with tf.name_scope('conv2.3.3') as scope:
            conv233 = tf.layers.conv2d(
                conv232,  # Input data
                filters=96,  # 32 filters
                kernel_size=(3, 3),  # Kernel size: 9x9
                strides=(1, 1),  # Stride: 1
            ...
            conv233 = tf.layers.batch_normalization(
                conv233,
                axis=-1,
                momentum=0.99,
                epsilon=epsilon,
            ...
            # apply relu
            conv233 = tf.nn.relu(conv233, name='relu2.3.3')

    # Branch 4
    with tf.name_scope('branch2.4') as scope:
        with tf.name_scope('pool2.4.1') as scope:
            pool241 = tf.layers.average_pooling2d(
                concat1,  # Input
                pool_size=(3, 3),  # Pool size: 3x3
                strides=(1, 1),  # Stride: 2
                padding='SAME',  # "same" padding
        ...
        with tf.name_scope('conv2.4.2') as scope:
            conv242 = tf.layers.conv2d(
                pool241,  # Input data
                filters=32,  # 32 filters
                kernel_size=(1, 1),  # Kernel size: 9x9
                strides=(1, 1),  # Stride: 1
            ...
            conv242 = tf.layers.batch_normalization(
                conv242,
                axis=-1,
                momentum=0.99,
                epsilon=epsilon,
            ...
            # apply relu
            conv242 = tf.nn.relu(conv242, name='relu2.4.2')

    # concatenate 2
    with tf.name_scope("concat2") as scope:
        concat2 = tf.concat(
            [ conv212, conv223, conv233, conv242],
            axis=3,
            name='concat2'
        )
    ...
    with tf.name_scope('pool2') as scope:
        ...
        # optional dropout
        if dropout:
            pool2 = tf.layers.dropout(pool2, rate=pooldropout_rate, seed=1000, training=training)
    ...
    with tf.name_scope('conv3') as scope:
        conv3 = tf.layers.conv2d(
            ...
            padding='SAME',  # "same" padding
            activation=None,  # None
            kernel_initializer=tf.truncated_normal_initializer(stddev=5e-2, seed=1100),
            kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=lamC),
            name='conv3'
        )
        ...
        if dropout:
            conv3_bn_relu = tf.layers.dropout(conv3_bn_relu, rate=convdropout_rate, seed=1200, training=training)
    ...
    with tf.name_scope('pool3') as scope:
        ...
        if dropout:
            pool3 = tf.layers.dropout(pool3, rate=pooldropout_rate, seed=1300, training=training)
    ...
    with tf.name_scope('conv4') as scope:
            conv4 = tf.layers.conv2d(
                ...
                padding='SAME',  # "same" padding
                activation=None,  # None
                kernel_initializer=tf.truncated_normal_initializer(stddev=5e-2, seed=1400),
                kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=lamC),
                name='conv4'
            )
            ...
            if dropout:
                conv4_bn_relu = tf.layers.dropout(conv4_bn_relu, rate=convdropout_rate, seed=1500, training=training)
